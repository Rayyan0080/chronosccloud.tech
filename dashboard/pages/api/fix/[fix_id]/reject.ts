...
export default async function handler(
  ...
  try {
    ...
    const rejectedEvent = {
      ...
      severity: 'warning',
      sector_id: sectorId,
      summary: `Fix ${fix_id} rejected: ${reason}`,
      correlation_id: correlationId,
      details: {
        ...fixDetails,
        rejected_by: 'operator-001',
    ...
    await collection.insertOne({
      topic: 'chronos.events.fix.rejected',
      payload: rejectedEvent,
      timestamp: new Date(rejectedEvent.timestamp),
      logged_at: new Date(),
    });

    // Try to publish to NATS (non-blocking - event is already in MongoDB)
    try {
      await publishToNATS('chronos.events.fix.rejected', rejectedEvent);
    } catch (natsError: any) {
      // Log but don't fail - event is already saved to MongoDB
      console.warn('NATS publish failed (event saved to MongoDB):', natsError.message);
    }

    res.status(200).json({
      success: true,
      message: `Fix ${fix_id} rejected`,
    });
  } catch (error: any) {
