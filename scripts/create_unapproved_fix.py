#!/usr/bin/env python3
"""
Create an unapproved test fix directly in MongoDB.
"""
...
def create_unapproved_fix():
    ...
    try:
        ...
        correlation_id = f"CRITICAL-{str(uuid4())[:8].upper()}"
        
        fix_details = {
            "fix_id": fix_id,
            "correlation_id": correlation_id,
            "source": FixSource.RULES.value,
            "title": "Auto-Generated Fix for Critical Event",
            "summary": "This fix was automatically generated for a Critical severity event. The system detected a critical issue and proposed this mitigation action.",
            "actions": [
                {
            ...
            "proposed_by": "fix-proposal-agent",
            "requires_human_approval": True
        }
        
        # Create fix.review_required event (NOT approved)
        fix_review_event = {
            "event_id": str(uuid4()),
            "timestamp": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
            "source": "fix-proposal-agent",
        ...
        print(f"[OK] Created unapproved fix in MongoDB!")
